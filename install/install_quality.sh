...
#   ./install_quality.sh -b                # 构建模式（仅构建镜像并推送到仓库）
...
REMOTE_IMAGES=(
    "${REGISTRY}/${REPOSITORY}/quality-server:latest"
    "${REGISTRY}/${REPOSITORY}/quality-frontend:latest"
)
...
while getopts "rubfh" opt; do
    case $opt in
        ...
        h)
            ...
            echo "  ./install_quality.sh -b        # 构建模式：仅构建镜像并推送到仓库"
...
if [ "$MODE" = "recover" ]; then
    ...
    echo -e "${YELLOW}模式: ${GREEN}构建模式 (仅构建镜像并推送到仓库)${NC}"
else
...
# 检查必要的文件
if [ ! -f "${MODULE_DIR}/Dockerfile_quality_server" ]; then
    echo -e "  ${RED}✗${NC} Dockerfile_quality_server 不存在"
    exit 1
fi

if [ ! -f "${MODULE_DIR}/Dockerfile_frontend" ]; then
    echo -e "  ${RED}✗${NC} Dockerfile_frontend 不存在"
    exit 1
fi

# 构建前端（升级模式总是重新构建以确保使用最新代码）
if [ "$MODE" = "upgrade" ] || [ "$MODE" = "recover" ]; then
...
# 使用本地基础镜像
echo -e "  ${YELLOW}使用本地基础镜像...${NC}"
echo -e "  ${GREEN}✓${NC} 基础镜像检查完成"
echo ""

# 构建 quality-server 二进制文件
# 使用动态链接，确保与服务器 libc 兼容
echo -e "  构建 quality-server 二进制文件..."
cd "${MODULE_DIR}"
...
# 步骤 2: 标记并推送镜像到仓库
# ============================================================
echo -e "${YELLOW}[步骤 2/4] 标记并推送镜像到仓库...${NC}"
echo ""

# 标记镜像
for i in "${!LOCAL_IMAGES[@]}"; do
    local_image=${LOCAL_IMAGES[$i]}
    remote_image=${REMOTE_IMAGES[$i]}
    
    echo -e "  标记镜像: ${local_image} -> ${remote_image}"
    docker tag "$local_image" "$remote_image"
    echo -e "  ${GREEN}✓${NC} 镜像标记完成"
    
    echo -e "  推送镜像到仓库..."
    push_output=$(docker push "$remote_image" 2>&1)
    if [ $? -eq 0 ]; then
        ...
        echo -e "  ${YELLOW}!${NC} 镜像推送失败，可能需要登录仓库"
...
if [ "$BUILD_ONLY" = true ]; then
    ...
    for remote_image in "${REMOTE_IMAGES[@]}"; do
        echo -e "  ${GREEN}$remote_image${NC}"
    done
    echo ""
    echo -e "${BLUE}在目标环境运行以下命令启动服务:${NC}"
    echo -e "  docker run -d --name quality-mysql --network quality-network \\
    -p 3306:3306 \\
    ...
    mysql:latest"
    echo ""
    echo -e "  docker run -d --name quality-server --network quality-network \\
    -p 5001:5001 \\
    ...
    /app/quality-server \\
    -addr :5001 \\
    -db \"root:root123456@tcp(quality-mysql:3306)/github_hub?parseTime=true\" \\
    -log-level info"
    echo ""
    echo -e "  docker run -d --name quality-frontend --network quality-network \\
    -p 8081:80 \\
    --restart unless-stopped \\
    ${REGISTRY}/${REPOSITORY}/quality-frontend:latest"
    echo ""
...
server_image="${REGISTRY}/${REPOSITORY}/quality-server:latest"
...
echo -e "  仅构建镜像:     ./install_quality.sh -b"
echo ""
