...
function EventHistory({ isLoggedIn }) {
  ...
  const getCheckTypeText = (checkType) => {
    ...
    const typeMap = {
      ...
      'ai_e2e': 'AI端到端测试',
      // 兼容 event-processor 使用的完整任务名称
      'deployment_deployment': '部署',
      'specialized_tests_api_test': 'API测试',
      'specialized_tests_module_e2e': '模块端到端测试',
      'specialized_tests_agent_e2e': 'Agent端到端测试',
      'specialized_tests_ai_e2e': 'AI端到端测试'
    }
                            ...
                            {selectedEvent.quality_checks
                              ...
                              .map(check => (
                                <div key={check.id} className="quality-check">
                                  ...
                                  {check.extra && (
                                    <div className="check-extra" style={{ marginTop: '8px', padding: '8px', backgroundColor: '#f8f9fa', borderRadius: '4px', fontSize: '12px' }}>
                                      <div style={{ fontWeight: 'bold', marginBottom: '4px', color: '#666' }}>额外信息:</div>
                                      <pre style={{ margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'monospace' }}>{check.extra}</pre>
                                    </div>
                                  )}
                                </div>
                              ))
