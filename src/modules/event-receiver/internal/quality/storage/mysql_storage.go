	...
	if len(eventIDs) > 0 {
		...
		checkQuery := `
			SELECT
				...
				duration_seconds, error_message, output, extra, retry_count,
		...
		for checkRows.Next() {
			...
			var errorMessage, output, extra sql.NullString
			...
			if err := checkRows.Scan(
				...
				&durationSeconds, &errorMessage, &output, &extra, &check.RetryCount,
...
func (s *MySQLStorage) createQualityCheckInTx(tx *sql.Tx, check *models.PRQualityCheck) error {
	result, err := tx.Exec(`
		INSERT INTO pr_quality_checks (github_event_id, check_type, check_status, stage, stage_order, check_order, started_at, completed_at, duration_seconds, error_message, output, extra, retry_count, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, check.GitHubEventID, check.CheckType, check.CheckStatus, check.Stage, check.StageOrder, check.CheckOrder, check.StartedAt, check.CompletedAt, check.DurationSeconds, check.ErrorMessage, check.Output, check.Extra, check.RetryCount, check.CreatedAt, check.UpdatedAt)
	if err != nil {
...
func (s *MySQLStorage) GetQualityCheck(id int) (*models.PRQualityCheck, error) {
	...
	err := s.db.QueryRow(`
		SELECT id, github_event_id, check_type, check_status, stage, stage_order, check_order, started_at, completed_at, duration_seconds, error_message, output, extra, retry_count, created_at, updated_at
		FROM pr_quality_checks
		WHERE id = ?
	`, id).Scan(
		&check.ID, &check.GitHubEventID, &check.CheckType, &check.CheckStatus, &check.Stage, &check.StageOrder, &check.CheckOrder, &startedAtTime, &completedAtTime, &durationSeconds, &errorMessage, &output, &extra, &check.RetryCount, &check.CreatedAt, &check.UpdatedAt,
	)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (s *MySQLStorage) UpdateQualityCheck(check *models.PRQualityCheck) error {
	_, err := s.db.Exec(`
		...
		SET check_status = ?, started_at = ?, completed_at = ?, duration_seconds = ?, error_message = ?, output = ?, extra = ?, updated_at = ?
		WHERE id = ?
	`, check.CheckStatus, check.StartedAt, check.CompletedAt, check.DurationSeconds, check.ErrorMessage, check.Output, check.Extra, check.UpdatedAt, check.ID)
	if err != nil {
...
func (s *MySQLStorage) BatchUpdateQualityChecks(checks []models.PRQualityCheck) error {
	...
	for _, check := range checks {
		_, err := stmt.Exec(
			...
			check.ErrorMessage, check.Output, check.Extra, check.UpdatedAt,
			check.ID,
		)
		if err != nil {
