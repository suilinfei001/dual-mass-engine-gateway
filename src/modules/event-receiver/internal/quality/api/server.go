...
// CreateSession 创建新会话
func (store *SessionStore) CreateSession(username string) (*Session, error) {
	store.mu.Lock()
...
func (s *Server) handleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	func() {
		...
		// 根据事件类型处理
		if eventType == "push" {
			result = s.pushHandler.Handle(payload)
		} else if eventType == "pull_request" {
			result = s.prHandler.Handle(payload)
		} else {
	...
	if result["status"] == "processed" {
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"event_db_id": result["event_db_id"],
		})
	} else {
...
func (s *Server) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	// 创建会话
	session, err := s.sessionStore.CreateSession(req.Username)
	if err != nil {
...
func (s *Server) handleCheckLogin(w http.ResponseWriter, r *http.Request) {
	...
	cookie, err := r.Cookie("session_id")
	if err == nil {
		session := s.sessionStore.GetSession(cookie.Value)
		if session != nil {
...
func (s *Server) handleBatchUpdateQualityChecks(w http.ResponseWriter, r *http.Request, eventID int) {
	...
	var updateData struct {
		QualityChecks []struct {
			...
			ErrorMessage *string `json:"error_message"`
			Output       *string `json:"output"`
			Extra        *string `json:"extra"`          // 额外信息（JSON字符串）
