...
func CreateChecksForEvent(githubEventID string) []PRQualityCheck {
	...
	checks = append(checks, PRQualityCheck{
		ID:              0,
		GitHubEventID:   githubEventID,
		CheckType:       QualityCheckTypeDeploymentAll,
		CheckStatus:     QualityCheckStatusPending,
	...
	// 专项测试流水线阶段 - 使用完整任务名称作为 CheckType（兼容 event-processor）
	specializedChecks := []struct {
		CheckType QualityCheckType
		Order     int
	}{
		{QualityCheckTypeSpecializedTestsAPITest, 1},
		{QualityCheckTypeSpecializedTestsModuleE2E, 2},
		{QualityCheckTypeSpecializedTestsAgentE2E, 3},
		{QualityCheckTypeSpecializedTestsAiE2E, 4},
	}
