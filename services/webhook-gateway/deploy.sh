...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Webhook Gateway 部署${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
cd "${SCRIPT_DIR}"
go build -mod=readonly -ldflags '-extldflags "-static"' -o output/webhook-gateway ./cmd/server
if [ $? -ne 0 ]; then
    echo -e "  ${RED}✗${NC} 编译失败"
    exit 1
fi
...
echo -e "  GitHub:      ${GREEN}http://localhost:${SERVICE_PORT}/webhook/github${NC}"
echo -e "  GitLab:      ${GREEN}http://localhost:${SERVICE_PORT}/webhook/gitlab${NC}"
echo ""
echo -e "${BLUE}配置信息:${NC}"
