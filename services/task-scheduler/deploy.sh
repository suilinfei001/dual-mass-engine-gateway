...
SERVICE_PORT=4003
EVENT_STORE_URL="http://localhost:4002"
EXECUTOR_URL="http://localhost:4004"
ANALYZER_URL="http://localhost:4005"
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Task Scheduler 部署${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
cd "${SCRIPT_DIR}"
go build -mod=readonly -ldflags '-extldflags "-static"' -o output/task-scheduler ./cmd/server
if [ $? -ne 0 ]; then
    echo -e "  ${RED}✗${NC} 编译失败"
    exit 1
fi
...
echo -e "  Event Store:   ${GREEN}${EVENT_STORE_URL}${NC}"
echo -e "  Executor:      ${GREEN}${EXECUTOR_URL}${NC}"
echo -e "  AI Analyzer:   ${GREEN}${ANALYZER_URL}${NC}"
echo ""
echo -e "${BLUE}常用命令:${NC}"
