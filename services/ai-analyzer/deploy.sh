...
CONTAINER_NAME="quality-${SERVICE_NAME}"
...
cd "${SCRIPT_DIR}"
go build -mod=readonly -ldflags '-extldflags "-static"' -o output/ai-analyzer ./cmd/server
if [ $? -ne 0 ]; then
    echo -e "  ${RED}✗${NC} 编译失败"
    exit 1
fi
...
# 构建环境变量参数
env_args="-e POOL_SIZE=${POOL_SIZE}"
if [ -n "$AI_IP" ]; then
    env_args="${env_args} -e AI_IP=${AI_IP}"
fi
if [ -n "$AI_MODEL" ]; then
    env_args="${env_args} -e AI_MODEL=${AI_MODEL}"
fi
if [ -n "$AI_TOKEN" ]; then
    env_args="${env_args} -e AI_TOKEN=${AI_TOKEN}"
fi
...
echo -e "  Pool Size:    ${GREEN}${POOL_SIZE}${NC}"
echo ""
echo -e "${BLUE}常用命令:${NC}"
...
echo ""
echo -e "${BLUE}环境变量:${NC}"
echo -e "  AI_IP        - AI 服务器地址"
echo -e "  AI_MODEL     - AI 模型名称"
echo -e "  AI_TOKEN     - AI 服务器 Token"
echo -e "  AI_POOL_SIZE - 请求池大小 (默认: 50)"
echo ""
