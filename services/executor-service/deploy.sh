...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Executor Service 部署${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
cd "${SCRIPT_DIR}"
go build -mod=readonly -ldflags '-extldflags "-static"' -o output/executor-service ./cmd/server
if [ $? -ne 0 ]; then
    echo -e "  ${RED}✗${NC} 编译失败"
    exit 1
fi
...
echo -e "  POST /api/execute"
echo -e "  GET  /api/executions/{id}"
echo -e "  GET  /api/executions/{id}/logs"
echo -e "  DELETE /api/executions/{id}"
echo ""
echo -e "${BLUE}常用命令:${NC}"
