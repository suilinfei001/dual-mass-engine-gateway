// Resource Manager Service - 资源池管理微服务
package main
...
import (
	...
	"github.com/quality-gateway/resource-manager/internal/api"
...
func init() {
	flag.StringVar(&port, "port", "4006", "Server port")
	...
	flag.StringVar(&dbName, "db-name", "resource_manager", "Database name")
...
func main() {
	...
	// 创建存储层
	resourceStorage := storage.NewResourceStorage(db)
	categoryStorage := storage.NewCategoryStorage(db)
	quotaPolicyStorage := storage.NewQuotaPolicyStorage(db)
	allocationStorage := storage.NewAllocationStorage(db)
	testbedStorage := storage.NewTestbedStorage(db)

	// 创建服务
	resourceService := service.NewResourceManagerService(
		resourceStorage,
		categoryStorage,
		quotaPolicyStorage,
		allocationStorage,
	...
	// 创建服务器
	srv := api.NewServer(serverConfig, resourceService, log)
